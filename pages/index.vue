<script setup lang="ts">
	import {
		Disclosure,
		DisclosureButton,
		DisclosurePanel,
	} from '@headlessui/vue';
</script>

<template>
	<div>
		<section
			class="bg-center bg-no-repeat hero-bg bg-gradient-to-t from-black/10 to-black via-black bg-blend-multiply"
		>
			<div class="px-4 mx-auto max-w-screen-lg text-center py-24 lg:py-56">
				<h1
					class="mb-4 text-4xl tracking-tight leading-none text-white md:text-5xl lg:text-7xl"
				>
					Delivering seamless logistics solutions for you
				</h1>
				<p
					class="mb-8 text-sm font-light text-white lg:text-base sm:px-16 lg:px-48"
				>
					Let us handle your logistics, so you can focus on what matters most.
				</p>

				<div class="flex gap-2 justify-center">
					<InputText
						type="text"
						size="large"
						placeholder="Input tracking ID"
						class="w-1/2"
					/>

					<Button
						label="Track"
						class="bg-primary-600 text-white focus:outline-none py-3 px-5 rounded-md"
					/>
				</div>
			</div>
		</section>

		<section class="relative overflow-hidden py-10 sm:py-24">
			<div class="container">
				<div class="grid grid-cols-1 gap-12 sm:grid-cols-2 lg:grid-cols-2">
					<div class="flex flex-col gap-10">
						<h2
							class="text-2xl font-semibold tracking-tight text-gray-900 sm:text-4xl"
						>
							We ship with speed domestically within Ghana and to and from the
							US
						</h2>

						<div class="flex flex-col text-center">
							<img
								src="/about-us-1.png"
								class="object-contain"
								alt="About us image"
							/>
						</div>
					</div>

					<div class="flex flex-col gap-10">
						<div class="flex flex-col text-center">
							<img
								src="/about-us-2.png"
								class="object-contain"
								alt="About us image"
							/>
						</div>
						<p class="text-base text-gray-700">
							Our global logistics expertise, advanced supply chain technology &
							customized logistics solutions will help you analyze, develop and
							implement successful supply chain management strategies from
							end-to-end.
						</p>

						<NuxtLink to="/">
							<Button
								class="bg-primary-600 uppercase flex gap-2 font-light text-white focus:outline-none py-3 px-5 rounded-md"
							>
								<span>About Us</span>
								<IconsArrowRight />
							</Button>
						</NuxtLink>
					</div>
				</div>
			</div>
		</section>

		<section class="relative overflow-hidden bg-[#EBEDF1] py-10 sm:py-24">
			<div class="container">
				<div class="grid grid-cols-1 gap-12 sm:grid-cols-2 lg:grid-cols-2">
					<div class="flex flex-col gap-16">
						<div class="flex flex-col gap-3">
							<h2
								class="text-2xl font-semibold tracking-tight text-gray-900 sm:text-4xl"
							>
								How We Help Businesses Across The World.
							</h2>
							<p class="text-base text-gray-600">
								Providing an independent advice and identifying the right fit
								for you. Services are sourced and procured based on solution
								specifications provided by our Supply Chain Solution.
							</p>
						</div>

						<div class="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-2">
							<div
								class="flex flex-col bg-primary-600 rounded text-white gap-2 py-8 px-6 transform transition duration-200 hover:scale-110 hover:shadow-2xl"
							>
								<div class="flex items-center">
									<IconsSmile />
								</div>
								<div class="flex flex-col gap-2">
									<h3 class="text-2xl text-white">98%</h3>
									<p class="text-sm text-white">Positive Feedback</p>
								</div>
							</div>

							<div
								class="flex flex-col bg-secondary-600 rounded text-white gap-2 py-8 px-6 transform transition duration-200 hover:scale-110 hover:shadow-2xl"
							>
								<div class="flex items-center">
									<IconsSmile />
								</div>
								<div class="flex flex-col gap-2">
									<h3 class="text-2xl text-white">20+</h3>
									<p class="text-sm text-white">Years of Experience</p>
								</div>
							</div>
						</div>
					</div>

					<div class="flex flex-col gap-2 items-start">
						<div class="flex gap-4 py-7 border-b border-gray-300 w-full">
							<div
								class="flex justify-center items-center w-12 h-12 border border-gray-300 rounded-full"
							>
								<img
									src="/icons/delivery-icon.svg"
									class=""
									alt=""
								/>
							</div>

							<Disclosure
								as="div"
								class="flex flex-col justify-start w-full"
								v-slot="{ open }"
							>
								<DisclosureButton
									class="py-2 text-xl font-medium flex gap-4 items-center justify-between w-full"
								>
									Is team pricing available?

									<div class="flex justify-end">
										<img
											src="/icons/chevronUpLight.svg"
											class=""
											alt=""
											v-if="open"
										/>

										<img
											src="/icons/chevronDownLight.svg"
											class=""
											alt=""
											v-else
										/>
									</div>
								</DisclosureButton>
								<DisclosurePanel class="text-gray-500">
									Yes! You can purchase a license that you can share with your
									entire team.
								</DisclosurePanel>
							</Disclosure>
						</div>

						<div class="flex gap-4 py-7 border-b border-gray-300 w-full">
							<div
								class="flex justify-center items-center w-12 h-12 border border-gray-300 rounded-full"
							>
								<img
									src="/icons/solution-icon.svg"
									class=""
									alt=""
								/>
							</div>

							<Disclosure
								as="div"
								class="flex flex-col justify-start w-full"
								v-slot="{ open }"
							>
								<DisclosureButton
									class="py-2 text-xl font-medium flex gap-4 items-center justify-between"
								>
									Customized solution

									<div class="flex justify-end">
										<img
											src="/icons/chevronUpLight.svg"
											class=""
											alt=""
											v-if="open"
										/>

										<img
											src="/icons/chevronDownLight.svg"
											class=""
											alt=""
											v-else
										/>
									</div>
								</DisclosureButton>
								<DisclosurePanel class="text-gray-500">
									Yes! You can purchase a license that you can share with your
									entire team.
								</DisclosurePanel>
							</Disclosure>
						</div>

						<div class="flex gap-4 py-7 border-b border-gray-300 w-full">
							<div
								class="flex justify-center items-center w-12 h-12 border border-gray-300 rounded-full"
							>
								<img
									src="/icons/chain-icon.svg"
									class=""
									alt=""
								/>
							</div>

							<Disclosure
								as="div"
								class="flex flex-col justify-start w-full"
								v-slot="{ open }"
							>
								<DisclosureButton
									class="py-2 text-xl font-medium flex gap-4 items-center justify-between"
								>
									Efficient supply chain solution

									<div class="flex justify-end">
										<img
											src="/icons/chevronUpLight.svg"
											class=""
											alt=""
											v-if="open"
										/>

										<img
											src="/icons/chevronDownLight.svg"
											class=""
											alt=""
											v-else
										/>
									</div>
								</DisclosureButton>
								<DisclosurePanel class="text-gray-500">
									Yes! You can purchase a license that you can share with your
									entire team.
								</DisclosurePanel>
							</Disclosure>
						</div>
					</div>
				</div>
			</div>
		</section>
	</div>
</template>

<style scoped>
	.hero-bg {
		background-image: url('/hero-1.png');
	}
</style>
