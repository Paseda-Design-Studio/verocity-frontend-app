	...
	<section>
		<section class="bg-primary-600 py-10 sm:py-24 relative">
			<div class="container relative z-20">
				<div class="flex items-center flex-col justify-center w-100 max-w-[1120px] mx-auto">
					...
					<div class="flex gap-2 w-full justify-center items-end mt-8 ">
						<div class="flex w-full flex-col gap-2">
							<label class="text-sm text-white" for="track">Track your shipment</label>
							<InputText type="text" size="large" name="track" placeholder="Input tracking ID"
								class="w-full flex-1" />
						</div>
						<Button label="Track" class="bg-secondary-600 text-white focus:outline-none py-4 px-8 rounded-md" />
					</div>
					<div class="flex mt-2">
						<p class="text-sm font-normal text-white">
							Need help changing your delivery?
							<NuxtLink to="/contact-us" class="font-semibold hover:underline">Get Help</NuxtLink>
						</p>
					</div>
				</div>
			</div>
			...
			<div class="container">
				<div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
					<div class="flex flex-col bg-[#FBFAF7] gap-4 py-10 px-8 transform transition duration-200 hover:scale-110 hover:shadow-2xl"
						v-for="item in lists" :key="item.title">
						<div class="flex justify-center items-center bg-gray-500 w-14 h-14 mx-auto rounded-lg"></div>
