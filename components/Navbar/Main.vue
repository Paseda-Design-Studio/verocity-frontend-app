...
<template>
	<header>
		<nav class="bg-white border-gray-200 dark:bg-gray-900">
			<div class="lg:container flex flex-wrap items-center justify-between mx-auto p-4">
				<NuxtLink to="/" class="flex items-center rtl:space-x-reverse">
					<img src="/logo/logo.png" class="w-[100px] h-auto" alt="Verocity Logo" />
				</NuxtLink>
				...
				<div class="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
					...
					<button class="py-1 px-2 bg-transparent text-secondary flex md:hidden focus:outline-none">
					...
					<ul
						...
						<li v-for="nav in navs" :key="nav.name">
							<NuxtLink :to="nav.link"
								...
								aria-current="page"><span>{{ nav.name }}</span></NuxtLink>
						</li>
					</ul>
				</div>
			</div>
